...
func (m *Repository) PostReservation(w http.ResponseWriter, r *http.Request) {
	...
	form.Required("first_name", "last_name", "email") // not include the validation of the "phone" in this time
...
func (m *Repository) PostShowLogin(w http.ResponseWriter, r *http.Request) {
	// This token prevents something known as such session fixation attack
	// Web app has a certain token associated with session.
	// RenewToken() is good practice in log in or log out.
...
func (m *Repository) Logout(w http.ResponseWriter, r *http.Request) {
	_ = m.App.Session.Destroy(r.Context())    // delete user_id by deleting session
	_ = m.App.Session.RenewToken(r.Context()) // regenerate token
...
func (m *Repository) AdminShowReservation(w http.ResponseWriter, r *http.Request) {
	...
	// get the reservation of the id from the database
...
func (m *Repository) AdminPostShowReservation(w http.ResponseWriter, r *http.Request) {
	err := r.ParseForm()
	if err != nil {
		// In admin page, you don't have to redirect home page like no-admin.
		// because admin page is only accessed by administrator
		helpers.ServerError(w, err)
		return
	}
...
func (m *Repository) AdminReservationsCalendar(w http.ResponseWriter, r *http.Request) {
	...
	for _, room := range rooms {
		...
		// log.Println(x.ID, firstOfMonth, lastOfMonth)
		...
		for _, restriction := range restrictions {
			if restriction.ReservationID > 0 {
				...
				// log.Println("reservationMap:", reservationMap)
			} else {
				// It's a block
...
func (m *Repository) AdminProcessReservation(w http.ResponseWriter, r *http.Request) {
	...
	err := m.DB.UpdateProcessedForReservation(id, 1) // 1: this reservation as processed, which is preserved
	if err != nil {
		log.Println(err)
	}
